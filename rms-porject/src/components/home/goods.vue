...
export default {
    ...
    input5(val, oldVal) {
      if (this.select == "") {
        ...
        if (
          ...
          this.$axios
            .get("http://localhost:5786/rms/title", {
              params: {
                select: this.select,
                item: this.input5
              }
            ...
            .then(res => {
              ...
              if (this.page) {
                this.$axios
                  .get("http://localhost:5786/rms/title/page", {
                    params: {
                      select: this.select,
                      item: this.input5,
                      page: this.page
          ...
          if (this.input5) {
            this.$axios
              .get("http://localhost:5786/rms/id", {
                params: {
                  id: this.input5
                }
              })
  ...
  methods: {
    ...
    handleDelete(index, row) {
      this.$axios
        .delete("http://localhost:5786/rms", {
          params: {
            id: row.id
          }
        })
    ...
    change() {
      this.$axios
        .post("http://localhost:5786/rms/change", {
